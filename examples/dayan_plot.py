...
flags.DEFINE_string('env', 'maxent_dayan', 'Environment to run.')
...
def plot_3d(env, v):
    ...
    value_map = construct_value_pred_map(env, v, contain_goal_value=True)

    # get rid of outer walls
    value_map = value_map[1:-1, 1:-1]
    # give values to inside walls
    value_map[np.isinf(value_map)] = np.median(value_map[~np.isinf(value_map)])

    value_map = np.rot90(value_map, k=3)
...
def main(argv):
    ...
    path = 'minigrid_basics/plots/dayan/'
    ...
    plt.figure(figsize=plt.figaspect(0.20))
    plt.subplot(1, 3, 1)
    plt.imshow(image)
    plt.axis('off')
    # plt.show()
    ...
    start_state = 59
    x = SR[start_state].reshape(h - 2, w - 2)
    # print(x.shape)
    # plot_3d(env, x)
    plt.subplot(1, 3, 2)
    plt.imshow(x)
    ...
    x = DR[start_state].reshape(h - 2, w - 2)
    plt.subplot(1, 3, 3)
    plt.imshow(np.log(x))
    # plot_3d(env, np.log(x))
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(join(path, f"{FLAGS.env}.png"), dpi=300)

    # x = MER[start_state].reshape(h - 2, w - 2)
    # plt.subplot(1, 4, 4)
    # plt.imshow(np.log(x))
    # # plot_3d(env, np.log(x))
    # plt.axis('off')
    ...
    # plt.figure(figsize=plt.figaspect(0.20))
    # plt.subplot(1, 2, 1)
    # plot_value_pred_map(env, DR_reward, contain_goal_value=True)

    # plt.subplot(1, 2, 2)
    # plot_value_pred_map(env, MER_reward, contain_goal_value=True)
    # plt.show()
