...
def compute_DR_TD(env, alpha=0.001):
    """
    Simply run uniform random policy
    """
    DR_closed_form = compute_DR(env)
    ...
    for e in range(1000):
        ...
        mae = np.abs(D - DR_closed_form).max()
        # print(np.argmax(np.abs(D - DR_closed_form)) // n_states)
        mae_list.append(mae)
        # if mae < 1e-6:
        #     break
...
def main(argv):
    ...
    D = compute_DR_TD(env, alpha=0.01)
    DR = compute_DR(env)
