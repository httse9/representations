...
def compute_rep_TD(env):
    """
    Function to learn representations using temporal-difference learning

    env: environment
    mode: which representation to learn, [SR, DR, MER]
    alpha: learning rate
    n_episodes: how many episodes to learn
    """
    mode = FLAGS.representation

    # tuned best learning rates for learning the representations.
    if "Empty" in env.unwrapped.spec.id:
        if mode == "SR":
            alpha = 0.1
        else:
            alpha = 0.03
    elif "FourRooms" in env.unwrapped.spec.id:
        if mode == "MER":
            alpha = 0.1
        else:
            alpha = 0.3

    n_episodes = FLAGS.rep_learn_n_episodes
...
def main(argv):
    ...
    path = join("minigrid_basics", "experiments", "reward_shaping_td", env.unwrapped.spec.id,)
