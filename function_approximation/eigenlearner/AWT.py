...
class AWTLearner(EigenLearner):
    """
    Asymmetric Weighted Transition
    - R^(-1) P
    """
    ...
    def update(self, step_size=0.001):
        ...
        for (s, a, r, s_next, r_next, terminated) in self.dataset:
            ...
            gradient[s] += -(np.exp(r) + np.exp(r_next)) * np.exp(self.eigvec[s_next] - self.eigvec[s])
        ...
        gradient += np.exp(2 * self.eigvec).sum() - 1       # normalization 1

        self.eigvec -= step_size * gradient

        ### explicit normalization of log vector
        
        ## method 1, normalize norm
        # norm = np.linalg.norm(np.exp(self.eigvec))
        # self.eigvec -= np.log(norm)

        ## method 2, anchor (doesn't work at all)
        # self.eigvec[self.env.terminal_idx[0]] = 0

"""
Only works for large lambda (e.g., 10, 20)
python -m minigrid_basics.function_approximation.eigenlearning_tabular --env gridroom_2 --n_epochs 100000 --step_size 1 --lambd 10
"""